...
```mermaid
graph TD
    Obliquity["Kemiringan Sumbu Bumi<br/>ε ≈ 23,4°"] --> Musim["Musim<br/>(sin a, ekstingsi, durasi siang)"]
    OrbitBulan["Orbit Bulan<br/>(e=0,055; i=5° thd ekliptika)"] --> Fase["Fase Bulan<br/>(bulan sinodis 29,53 hari)"]
    OrbitBulan --> Node["Node Orbit Bulan<br/>(presesi 18,6 tahun)"]
    Fase --> Gerhana["Syarat Gerhana<br/>(fase + dekat node)"]
    Node --> Gerhana
    Gerhana --> JenisGerhana["Total / Sebagian / Cincin / Penumbra"]
    Gerhana --> Saros["Siklus Saros<br/>223 bulan sinodis ≈ 242 bulan nodical"]
    Fase --> Meton["Siklus Meton<br/>19 tahun ≈ 235 bulan sinodis"]
    Meton --> KalenderLunisolar["Kalender Lunisolar<br/>(Bab III)"]
    TidalForce["Gaya Pasang Surut<br/>(Bab IV)"] --> PasangSurut["Pasang Purnama vs Perbani"]
    TidalForce --> TidalLock["Tidal Locking Bulan"]
    Barycenter["Barycenter Bumi-Bulan<br/>(≈4670 km dari pusat Bumi)"] --> Librasi["Librasi (59% permukaan terlihat)"]
    Albedo["Albedo Bond & Geometris"] --> FaseKecerlangan["Fungsi Fase Φ(α)"]
    FaseKecerlangan --> Earthshine["Earthshine"]
    FaseKecerlangan --> MagnitudoBulan["Magnitudo Bulan vs Fase"]
```
